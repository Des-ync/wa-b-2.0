...
-- =========================================================================
-- webauthn_credentials / webauthn_challenges: retired. Passkeys are now
-- handled entirely by Clerk (user.createPasskey() / authenticateWithPasskey()
-- client-side) instead of our own @simplewebauthn/server ceremony — see
-- src/routes/auth.routes.js and public/mobile-clerk-bridge.js. Dropped
-- rather than left in place since the feature never worked in production
-- (Node 18 had no global WebCrypto until the day before this migration),
-- so there is no real credential data to preserve.
-- =========================================================================
