...
/**
 * Resolve the outbound messaging adapter for a channel. All adapters expose
 ...
 *
 * Instagram AND Messenger run in TEXT MODE: quick-reply chips vanish as soon
 * as the conversation moves on and never render on desktop (same underlying
 * Meta Send API on both platforms, same UX problem), so buttons and lists go
 * out as numbered text menus instead. The options shown are remembered in
 * conversation_state.flow_data.ig_options (shared key — channel-agnostic
 * storage, name predates Messenger support), and the conversation handler
 * translates a typed "2" back into the second option's interactive id.
 */
function getAdapter(channel) {
  if (channel === 'instagram') return textMode(ig);
  if (channel === 'messenger') return textMode(messenger);
  return wa;
}
...
/**
 * Wrap a channel service (Instagram or Messenger — same Send API shape) so
 * buttons/lists render as numbered text instead of native quick replies.
 */
function textMode(service) {
  return {
    ...service,
    ...
    async sendButtons(to, body, buttons = [], meta = {}) {
      ...
      const res = await service.sendText(to, text, meta);
...
/**
 * The channel-native destination for a customer row: the channel-scoped user
 * id for Instagram/Messenger, the WhatsApp number (unchanged, backward-
 * compatible) otherwise.
 */
function destOf(customer) {
  return ['instagram', 'messenger'].includes(customer?.channel)
    ? customer.channel_id
