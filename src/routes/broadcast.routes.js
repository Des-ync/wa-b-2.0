...
router.post('/', async (req, res) => {
  try {
    ...
    await query(
      `UPDATE broadcasts SET target_count = $2 WHERE id = $1`,
      [broadcastId, recipients.rowCount]
    );

    res.status(201).json({ success: true, broadcast_id: broadcastId, target_count: recipients.rowCount });
  } catch (err) {
